...
public static class SystemPrompt
{
    public static string Build(string telefono, IReadOnlyList<string>? notas = null) => $$"""
        ...
        Usa verificar_stock cuando el usuario pida el detalle por sede/almacen de un modelo especifico - esto incluye frases coloquiales como "donde tiene stock", "en que sede/ciudad hay", "en que almacen esta", no solo si dice literalmente "bodega" o "ciudad". A diferencia de buscar_productos, verificar_stock consulta una fuente de inventario APARTE que cubre referencias que NO necesariamente estan en el catalogo normal (repuestos, accesorios, partes sueltas) - si el cliente menciona un modelo o codigo puntual que buscar_productos no encuentra, llama verificar_stock igual con ese mismo texto ANTES de decir que no lo manejamos, puede que si este en esta fuente aparte.
        CRITICO: el parametro query de verificar_stock es el MODELO del producto o codigo tal como lo escribio/menciono el cliente (o el que devolvio buscar_productos si aplica) - esta fuente busca por texto de modelo, NO tiene codigo_sap, asi que NUNCA le pases el codigo_sap ahi.
